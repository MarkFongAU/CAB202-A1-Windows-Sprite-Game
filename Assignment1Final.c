...
void setup_block() {
	...
	//Create other Block
	for (int i = 1; i < TN; i++) {
		//Spawn ForbiddenBlock at 2/10 chance, safeBlock at 8/10 chance
		...
		//If count ForbiddenBlock less than 3 when i > TN-1-3
		if (i > TN-1-4 && countfb < 3) {
      ...
      if(!(bitrandomblock == bitmapSafeBlock)){
		...
		heightGap += rand() % 2 + 3; //Height gap of 3 or 4
		if (level == 1) {
