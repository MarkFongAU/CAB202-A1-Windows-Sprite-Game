...
#define TN 16 //Number of blocks in game
...
void contain_humaninScreen() {
	...
	if(level==2 || level==3)draw_formatted(22, 1, "Horizontal Velocity: %d", (int)jx);
	else draw_formatted(22, 1, "                       ");
	//show_screen();
}
/*
...
void draw_displayscreen() {
	...
	if(level==3)draw_string(max_x - 15, max_y - 1, "Speed :");
	else draw_string(max_x - 15, max_y - 1, "       ");
	...
	else draw_string(max_x / 2 + 15, max_y - 1, "   ");
...
void setup_block() {
  ...
  preheightLocation = first_y;

	//Height between tiles
	int heightGap = 0;
  ...
  //Count SafeBlock
	int countsb = 0;
  //Check block are at same row, must go to next heightGap
  blockcountinrow = 2;
	...
	for (int i = 1; i < TN; i++) {
      ...
      countsb++;
		...
		//If count ForbiddenBlock less than 2 when i > TN-1-3
		if (i > TN-1-3 && countfb < 2) {
      if(!(bitrandomblock == bitmapForbiddenBlock)){
        bitrandomblock = bitmapForbiddenBlock;
        countfb++;
      }

		}
    else if (i> TN-1-5 && countsb < 5){  //If count SafeBlock less than 5 when i > TN-1-5 (Unlikely)
      if(!(bitrandomblock == bitmapSafeBlock)){
        bitrandomblock = bitmapSafeBlock;
  			countsb++;
      }
    }
		...
		if(rand() % 10 + 1 > 8)heightGap += rand() % 2 + 3;
...
void create_destruction_bitmap(char** bitmap, char* bitdesc) {
	...
	*bitmap = malloc(25 * sizeof *bitmap);
	//sprintf(*bitmap, " vvv vvvvvvvvvvvvvvv vvv " bitdesc); //5
  sprintf(*bitmap, " vvv vvvvvvvvvvvvvvv vvv "); //5
}
/*
* destroy_destruction_sprite
*/
